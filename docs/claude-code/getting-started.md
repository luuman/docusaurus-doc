...
本章节将引导你完成 Claude Code 的安装、模型配置和基础使用，涵盖国内外主流模型的接入方式。

---

## 前置准备

在安装 Claude Code 之前，请确认以下依赖已就绪：

| 依赖 | 最低版本 | 说明 |
|------|---------|------|
| **Node.js** | v18.0+ | Claude Code 的运行时环境 |
| **Git** | v2.0+ | 版本控制集成所需 |
| **API Key** | — | Anthropic 官方或国内兼容模型的 API 密钥 |

:::tip 获取 API Key
- **Anthropic 官方**：前往 [console.anthropic.com](https://console.anthropic.com/) 创建 API Key
- **国内模型**：参考下方 [常用国内模型配置](#常用国内模型配置) 表格中的获取途径
:::
...
```
Haiku 4.5  →  Sonnet 4.5  →  Opus 4.5 + Thinking
（快速/低成本）  （性价比首选）   （最高质量/复杂推理）
```

- **Haiku 4.5**：适合简单查询、格式转换、快速补全
- **Sonnet 4.5**：日常开发的最佳平衡点，推荐作为默认模型
- **Opus 4.5 + Thinking**：复杂架构设计、深度代码审查、需要多步推理的任务
...
1. **Skills** — 学习如何将常用工作流封装为自定义斜杠命令
2. **Hooks** — 了解如何在代理执行的关键节点注入自定义逻辑
3. **MCP Servers** — 将数据库、API 等外部资源接入 Claude Code
4. **Subagents** — 探索多代理并行协作模式，处理大型复杂任务
